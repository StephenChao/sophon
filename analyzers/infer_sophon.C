...
void infer_sophon(TString inputFile, TString outputFile, TString modelPath, TString jetBranch = "JetPUPPIAK8", bool debug = false, bool keepInfo = true) {
    ...
    TClonesArray *branchJetAK4 = treeReader->UseBranch("JetPUPPI");
    ...
    // Keep initial tree infomation
    if (keepInfo){
        tree->Branch("JetPUPPI", &branchJetAK4);  
        tree->Branch("Particle", &branchParticle);   
        tree->Branch(jetBranch, &branchJet);  
    }
    ...
    // if for test, then only use 10 events as below
    // allEntries = 10; 
